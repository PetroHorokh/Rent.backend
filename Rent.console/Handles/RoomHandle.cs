...
public static class RoomHandle
{
    ...
    private static async Task AddAccommodationRoom()
    {
        ...
        Console.WriteLine(result.Error != null ? '\n' + result.Error.Message : "\nSuccessfully added a accommodation to room");
    }

    private static async Task ChangeQuantityOfAccommodation()
    {
        ...
        if (Guid.TryParse(input, out Guid accommodationRoomId))
        {
            ...
            if (accommodation != null)
            {
                ...
                var result = await RoomService.ChangeQuantityOfAccommodationAsync(accommodationRoomToUpdateDto);
